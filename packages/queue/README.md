...
### The two codecs do not agree on what a handler receives

`Codec\Json` flattens an object on the way out and `json_decode(assoc: true)` returns an array on the way back. `Codec\Igbinary` preserves the class. So the same payload arrives as two different types depending on which writer is configured, and a consumer written against one breaks on the other — with no compile-time warning, and no failing test, because a test over the JSON codec passes either way.

That is worth knowing before flipping a writer. A consumer whose handlers rebuild from arrays — `new Document($payload['project'])`, where `Document extends ArrayObject` — runs for years on JSON and wedges within minutes of the flip: every delivery of every affected message throws a `TypeError`, each holding a `maxAckPending` slot through its whole backoff, until the consumer has no slot left to deliver into.

`Codec\Plain` makes the payload's shape a property of the payload rather than of the format, in both directions:
...
An `ArrayObject` becomes its storage and a `stdClass` becomes an array, at any depth — the two shapes an envelope actually carries, a document-shaped value and the empty map a JSON renderer leaves behind for `{}`. Any other class passes through untouched: a codec quietly reshaping a type nobody considered is how this class of defect is made, not how it is fixed.

Reading matters as much as writing, which is why it flattens both ways. Bytes outlive the build that wrote them — queued, in flight, and on dead letters that have no deadline — so a message written before this was composed in, or by a pod that has not rolled yet, still reaches a handler as arrays.

Nothing wraps anything on its own. A consumer that means to round-trip objects through igbinary is doing something this package supports, and composing `Plain` is how the other kind says so.
